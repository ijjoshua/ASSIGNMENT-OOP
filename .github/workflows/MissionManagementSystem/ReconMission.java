...
public class ReconMission extends Mission {
    ...
    public void allocateResources(Resource resource) throws ResourceAllocationException {
        if (!resource.getResourceType().equals("RECON_EQUIPMENT")) {
            throw new ResourceAllocationException("Invalid resource type for reconnaissance mission");
        }
    ...
    public String generateMissionReport() {
        ...
        report.append("Reconnaissance Mission Report\n");
    ...
    public boolean validatePersonnel() {
        return assignedPersonnel.size() >= 2;
    }

    public boolean validateResources() {
        return allocatedResources.stream().anyMatch(r -> r.getResourceName().equals("Drone"));
    }
}