...
public class RescueMission extends Mission {
    ...
    public void allocateResources(Resource resource) throws ResourceAllocationException {
        if (!resource.getResourceType().equals("MEDICAL_SUPPLIES") &&
                !resource.getResourceType().equals("RESCUE_EQUIPMENT")) {
            throw new ResourceAllocationException("Invalid resource type for rescue mission");
        }
    ...
    public boolean validatePersonnel() {
        return assignedPersonnel.stream().anyMatch(p -> p.getPersonnelRole().equals("Medic"));
    }

    public boolean validateResources() {
        return allocatedResources.stream().anyMatch(r -> r.getResourceName().equals("Medical Kit"));
    }
}
